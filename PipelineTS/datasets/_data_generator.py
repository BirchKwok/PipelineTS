...
class DataGenerator:
    ...
    def trigonometry_ds(size: int = 1000, sin_cos_noise_fact: Sized = (0.5, 0.3, 0.2),
        ...
        numpy.ndarray
        """
        raise_if_not(ValueError, size is not None and isinstance(size, (int, float)) is True,
                     "size must be int or float")
        raise_if_not(ValueError, random_state is None or isinstance(random_state, int) is True,
                     "random_state must be int or None")
        raise_if_not(ValueError, len(sin_cos_noise_fact) == 3, "sin_cos_noise_fact must be a tuple of 3 floats")
        np.random.seed(random_state)
        s = int(np.ceil(size))

        raise_if_not(ValueError, s > 0, "size must be larger than 0")
    ...
    def white_noise(size=1000, mean=0., std=1., random_state=None):
        ...
        raise_if_not(ValueError, random_state is None or isinstance(random_state, int) is True,
                     "random_state must be int or None")
        raise_if_not(ValueError, isinstance(mean, float) is True and isinstance(std, float) is True,
                     "mean and std must be float")
    ...
    def random_walk(size=1000, started_zero=True, random_state=None):
        ...
        raise_if_not(ValueError, random_state is None or isinstance(random_state, int) is True,
                     "random_state must be int or None")
        raise_if_not(ValueError, isinstance(started_zero, bool) is True, "started_zero must be bool")
...
class RandomEventGenerator:
    ...
    def __init__(self, size=10000, seasons=None, sin_cos_noise_fact=(0.5, 0.3, 0.2), stacking_level=6,
                 random_state=None):
        raise_if_not(ValueError, size is not None and isinstance(size, (int, float)) is True,
                     "size must be int or float")

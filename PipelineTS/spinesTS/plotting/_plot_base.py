...
def plot1d(*S: Sequence, x: Optional[Sequence] = None,
    ...
    if isinstance(colors, str) or colors is None:
        ...
        raise_if_not(TypeError, isinstance(colors, (list, tuple)), "colors must be a list or tuple")
        raise_if_not(ValueError, len(colors) == len(S), "colors length must be equals to sequence length")
    ...
    for i in range(seq_len):
        if labels is not None:
            if isinstance(labels, str):
                ...
                raise_if_not(ValueError, len(labels) == seq_len,
                             "labels length must be equals to sequence length when labels is a list")
                label = labels[i]
        else:
...
def plot2d(*S: Sequence, x: Optional[Sequence] = None,
    ...
    matplotlib.figure.Figure
    """
    raise_if_not(ValueError, len(S) > 0, "S must be a non-empty sequence")
    raise_if_not(ValueError, np.sum([np.ndim(i) == 2 for i in S]) == len(S), "S must be a sequence of 2-d array-like")
    raise_if_not(TypeError, isinstance(eval_slices, str), "eval_slices must be a string")

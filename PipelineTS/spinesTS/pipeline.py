...
from PipelineTS.spinesTS.base import ForecastingMixin
...
class Pipeline(ForecastingMixin):
    ...
    def __init__(self, steps: List[tuple]):
        ...
        Demo:
            ...
            from PipelineTS.spinesTS.preprocessing import split_array
            from PipelineTS.spinesTS.data import LoadElectricDataSets
            from sklearn.preprocessing import StandardScaler
            from PipelineTS.spinesTS.nn import TCN1D
            ...
            '''
        """
        raise_if_not(ValueError, isinstance(steps, list), "steps must be a list")
        raise_if_not(ValueError, all([isinstance(i, tuple) for i in steps]), "steps must be a list of tuples"
                                                                             "with (name, estimator) format")
        raise_if_not(ValueError, 0 < len(steps) == np.sum([isinstance(i, tuple) for i in steps]),
                     "the length of steps must be larger than 0 and equal to the number of tuples in steps")

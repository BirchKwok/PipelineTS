...
from spinesUtils.asserts import check_has_param, raise_if_not
...
class MultiStepRegressor(ForecastingMixin):
    ...
    def fit(self, x, y, eval_set=None, **estimator_fit_kwargs):
        raise_if_not(ValueError, np.ndim(y) <= 2, "y must be a 1d or 2d array")
    ...
    def predict(self, x, **kwargs):
        ...
        for step in range(self._forward):  # forward n steps
            ...
            raise_if_not(ValueError, x.shape == eval_x.shape, "x and eval_x must have the same shape")
...
class MultiOutputRegressor(ForecastingMixin, _MultiOutputEstimator):
    ...
    def fit(self, X, y, eval_set=None, **fit_params):
        ...
        **fit_params : dict of string -> object
        ...
        fit_params_validated = fit_params
...
class MultivariableMultiOutputRegressor(ForecastingMixin):
    ...
    def check_x_y(self, x, y):
        raise_if_not(ValueError, x.shape[0] == y.shape[0], "x and y must have the same number of samples")
        raise_if_not(TypeError, isinstance(x, (np.ndarray, torch.Tensor)) and isinstance(y, (np.ndarray, torch.Tensor)),
                        "only accept numpy.ndarray or torch.Tensor")
        raise_if_not(ValueError, x.ndim == 2, "x must be a 2d array")

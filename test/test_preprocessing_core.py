"""
Comprehensive test suite for PipelineTS preprocessing and metrics utilities.

Tests:
- split_series, train_test_split_ts, lag_splits, split_series_multivariate
- GaussRankScaler, MultiDimScaler
- moving_average
- PipelineTS metrics: wmape, rmse, mae, mse
"""
...
class TestSplitSeries:
    def test_basic_split(self):
        from PipelineTS.preprocessing import split_series
...
class TestTrainTestSplitTS:
    def test_basic_split(self):
        from PipelineTS.preprocessing import train_test_split_ts
...
class TestLagSplits:
    def test_basic(self):
        from PipelineTS.preprocessing import lag_splits
        data = np.arange(20, dtype=np.float32)
        X = lag_splits(data, 5)
        assert X.shape[0] > 0
        assert X.shape[1] == 5
...
class TestGaussRankScaler:
    def test_fit_transform(self):
        from PipelineTS.preprocessing import GaussRankScaler
...
class TestMovingAverage:
    def test_basic(self):
        from PipelineTS.preprocessing import moving_average
...
class TestPipelineTSMetrics:
    ...
    def test_mse(self):
        from PipelineTS.metrics import mse
        y_true = np.array([1.0, 2.0, 3.0])
    ...
    def test_wmape(self):
        from PipelineTS.metrics import wmape
    ...
    def test_wmape_loss_torch(self):
        import torch
        from PipelineTS.metrics import WMAPELoss
        loss_fn = WMAPELoss()
    ...
    def test_rmse_loss_torch(self):
        import torch
        from PipelineTS.metrics import RMSELoss
        loss_fn = RMSELoss()
        inputs = torch.tensor([1.0, 2.0, 3.0])
